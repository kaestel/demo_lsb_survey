...
UseDetector = new function() {
	...
	this.include = function() {
		...
		if((this.getParam("dev") && this.getParam("dev") != "0") || (this.getCookie("dev") && this.getCookie("dev") != "0")) {
			document.write('<script type="text/javascript" src="'+host+'/js/lib/seg_'+segment+'_include.js"></script>');
			document.write('<link type="text/css" href="'+host+'/css/lib/seg_'+segment+'_include.css" rel="stylesheet" media="all" />');
		}
		else {
			document.write('<script type="text/javascript" src="'+host+'/js/seg_'+segment+'.js"></script>');
			document.write('<link type="text/css" href="'+host+'/css/seg_'+segment+'.css" rel="stylesheet" media="all" />');
		}
...
Detector = new function() {
	...
	this.getSegment = function(ua) {
		...
		groups["desktop_ie9"] = "desktop";
		groups["tablet"] = "desktop";
		groups["tablet_light"] = "desktop";
		groups["tv"] = "desktop";

		groups["smartphone"] = "smartphone";

		groups["desktop_light"] = "unsupported";
		groups["mobile"] = "unsupported";
		groups["mobile_light"] = "unsupported";
		groups["seo"] = "unsupported";
	...
	this.identify = function(ua) {
		...
		if(!ua.match(/(ipad|ipod|mobile|android|fennec|phone|symbian|midp|kindle|smart-tv)/i)) {
			...
			else if(!ua.match(/(chromeframe|msie|trident|edge\/1[2-5])/i) && ua.match(/((chrome|chromium)\/(2[5-9]|[3-6][0-9])\b)/i)) {
				return "desktop_edge";
			}
			else if(!ua.match(/(msie|trident)/i) && ua.match(/(firefox\/(2[2-9]|[3456][0-9])\b)/i)) {
				return "desktop_edge";
			}
			else if(!ua.match(/(msie|lumia)/i) && ua.match(/(windows NT[^$]+edge\/1[2-5])/i)) {
				return "desktop_edge";
			}
		...
		if(ua.match(/(Wii|Playstation \d|smart-tv|TABLO|mbox|Liberate DTV)/i)) {
			return "tv";
		}
		...
		if(!ua.match(/(midp)/i) && ua.match(/(ipad|webkit|android|tablet)/i)) {
			...
			else if(ua.match(/(tablet[^$]+firefox\/(2[2-9]|[345][0-9])\b)/i)) {
				return "tablet";
			}
			else if(!ua.match(/(padfone|^(?!.*(chrome\/[3-9][0-9]))|^(?!.*android)|dalvik)/i) && ua.match(/((nexus (7|9|10))|tablet|(HTC[^$]*(flyer|jetstream|p51[0-2]{1}))|(xoom|MZ[5-6][0-9]{2})|(lifetab|[SP]10[035]{2}X)|(folio_|AT(7|10|[1237][037]0))|(one[ _\-]?touch[ _\-]?(P[0-9]{3}|tab|evo|t10|9005)|i213|900[26][XW])|( slate[ ]?(7|8|9|10|12|17|21)|slatebook|TouchPad)|(RCT6)|( n1 )|(archos ([7-9]|10)([0-9]))|(mediapad|m2-[A8]0[12][LW])|viewpad|PAD-FMD700|(nook|BNTV)|(SGP[T0-9]{3}|SO-0(3E|5F))|(SM-(T|P)[0-9]{3}|GT-(P[0-9]{4}|N(5|8)[0-9]{3})|SGH-(I(467|497|957)|T779)|SCH-I(705|800|815|905|915|925)|SPH-P500|SHW-M480)|(NX(0|7))|(; arc b)|(Kindle fire| KF[A-Z]{2,4} b)|hudl|(Edison|Aquaris E10)|(; A[1257][0-9]{2} b|A1-(7|8)[0-9]{2}|A3-A[0-5]{2} b|B[13]-[A0-9]{3})|(LG-(V|UK)[4-9][0-9]{2})|(en-tab|TAB-E419)|(QMV7|Smart[ ]?Tab|QTAIR)|(TA[BCDQ]-[0-9]{5})|Cintiq|(Endeavour|Discovery|Polaris_808|Atlantis 1001)|polypad|[ _-]E(8|9|10)[QT]|(; (T[1-5][0-9]{2}|R101|PS47) )|(tab-protab)|(tab-p)|(; b-tab)|(lenco[^$]+tab-(74|92))|(iq(tab|(3|7|8|9)[0-9]{3}|[GC]189|10[0-9]{2}|13[0-9]{2}|18[0-5][0-9]))|freetab|mytab|(tab(1004|[7-8][0-4]4))|( tab [1-9][0-9]{2})|( TAB7[0-9]{2})|(e-tab|etab5)|(vtab)|(i-life wtab )|(@tab-(seven|eight|nine|ten|thirteen))|(APPTAB(7|9))|((xenta|noble|luna) tab)|tolino tab|(AG Chrome Go Tab)|(RoverPad)|EvoPAD|PocketBook|(transformer|TF300|Slider SL|Memo pad|; K0[0-1][A-Z0-9] b|ME(1|3|5)[0-9]{2}| P0[0-2][1-4ACZTYM])|(mi pad)|((ideatab|lenovo)[ _-]?(a[1-7][0-6][079]|b[68]0[08]0|S6000|TAB|TB|YT|YB)|ideapada10|Tab2a7)|(LaVieTab)|SurfTab|eZee'Tab|LineTab(7|10)|TAB950|TAB917|WEXLER-TAB|Voda(fone|com)[^$]+Tab|VF-13|VFD 1100|; V_TAB|Tegra[ _]?Note|Pad971|venue[^$]+(7|8|1[0-5])|Le Pan|HSG1310|Kliver7806|TGW[- ]?(7|8|10)|BLASTER|NS-(P[0-9]{2}A|1[45][AT])|( 9020A )|( DL(7|8|10)[0-9]{2}[MQDAW])|T8-PLUS|Bush Spira B[12]|( I960 )|NOON[ _]PRO|ADVAN T5C|AT[178][CASE]|( B899 ))/i)) {
				return "tablet";
			}
			...
			else if(!ua.match(/(chrome\/[3-9][0-9]|webkit\/[3-4]|^(?!.*android)|opera|msie|firefox|dalvik)/i) && ua.match(/((nexus (7|9|10))|tablet|(HTC[^$]*(flyer|jetstream|p51[0-2]{1}))|(xoom|MZ[5-6][0-9]{2})|(lifetab|[SP]10[035]{2}X)|(folio_|AT(7|10|[1237][037]0))|(one[ _\-]?touch[ _\-]?(P[0-9]{3}|tab|evo|t10|9005)|i213|900[26][XW])|( slate[ ]?(7|8|9|10|12|17|21)|slatebook|TouchPad)|RCT6|( n1 )|(archos ([7-9]|10)([0-9]))|(mediapad|m2-[A8]0[12][LW])|viewpad|PAD-FMD700|(nook|BNTV)|(SGP[T0-9]{3}|SO-0(3E|5F))|(SM-(T|P)[0-9]{3}|GT-(P[0-9]{4}|N(5|8)[0-9]{3})|SGH-(I(467|497|957)|T779)|SCH-I(705|800|815|905|915|925)|SPH-P500|SHW-M480)|(NX(0|7))|(; arc b)|(Kindle fire| KF[A-Z]{2,4} b)|hudl|(Edison|Aquaris E10)|(; A[1257][0-9]{2} b|A1-(7|8)[0-9]{2}|A3-A[0-5]{2} b|B[13]-[A0-9]{3})|(LG-V[4-9][0-9]{2})|(en-tab|TAB-E419)|(QMV7|Smart[ ]?Tab|QTAIR)|(TA[BCDQ]-[0-9]{5})|Cintiq|(Endeavour|Discovery|Polaris_808|Atlantis 1001)|polypad|[ _-]E(8|9|10)[QT]|(; (T[1-5][0-9]{2}|R101|PS47) )|(tab-protab)|(tab-p)|(; b-tab)|(lenco[^$]+tab-(74|92))|(iq(tab|(3|7|8|9)[0-9]{3}|[GC]189|10[0-9]{2}|13[0-9]{2}|18[0-5][0-9]))|freetab|mytab|(tab(1004|[7-8][0-4]4))|( tab [1-9][0-9]{2})|( TAB7[0-9]{2})|(e-tab|etab5)|vtab|(i-life wtab )|(@tab-(seven|eight|nine|ten|thirteen))|(APPTAB(7|9))|((xenta|noble|luna) tab)|(tolino tab)|(AG Chrome Go Tab)|RoverPad|EvoPAD|PocketBook|(transformer|TF300|Slider SL|Memo pad|; K0[0-1][A-Z0-9] b|ME(1|3|5)[0-9]{2}| P0[0-2][1-4ACZTYM])|mi pad|((ideatab|lenovo)[ _-]?(a[1-7][0-6][079]|b[68]0[08]0|S6000|TAB|TB|YT|YB)|ideapada10|Tab2a7)|LaVieTab|SurfTab|eZee'Tab|LineTab(7|10)|TAB950|TAB917|WEXLER-TAB|; V_TAB|Tegra[ _]?Note|Voda(fone|com)[^$]+Tab|VF-13|VFD 1100|Pad971|venue[^$]+(7|8|1[0-5])|Le Pan|HSG1310|Kliver7806|TGW[- ]?(7|8|10)|BLASTER|NS-(P[0-9]{2}A|1[45][AT])|( 9020A )|( DL(7|8|10)[0-9]{2}[MQDAW])|T8-PLUS|Bush Spira B[12]|( I960 )|NOON[ _]PRO|ADVAN T5C|AT[178][CASE]|( B899 ))/i)) {
				return "tablet_light";
			}
		}
		if(!ua.match(/(msie|midp|symbian)/i) && ua.match(/(android|webkit|iphone|windows phone)/i)) {
			...
			else if(!ua.match(/(dalvik|trident|windows phone|LG-E400|LG-D125|SM-G110)/i) && ua.match(/(android[^$]+AppleWebKit\/(53[4-9]|6[0-9][0-9]))/i)) {
				return "smartphone";
			}
			...
			else if(ua.match(/(touch[^$]+IEMobile\/11|Windows Phone[^$]+Edge\/1[1-5]|windows nt 10[^$]+lumia[^$]+Edge\/1[1-5])/i)) {
				return "smartphone";
			}
		}
		...
		else if(!ua.match(/(phone|mobile|ipad|android|tablet|midp|wap|windows[ ]?ce|\d\d\dx\d\d\d|symbian)/i) && ua.match(/(windows[\+ ]?(9[58]|xp|nt)|firefox|chrome|webkit|Macintosh|x11|wpdesktop|win[\+ ]?(9[58x]|xp|nt)|excel|powerpoint|outlook|aol|safari|Camino|gecko|Amiga)/i)) {
			return "desktop_light";
		}
		else if(ua.match(/(ipad|tablet|LIFETAB|ideatab|NewTab CS1|venue[^$]+(7|8|1[0-5]))/i)) {
			return "tablet_light";
		}
		else if(ua.match(/(iphone|android|webkit)/i)) {
			return "mobile";
		}
		else if(!ua.match(/(http[s]?:|@)/i) && ua.match(/(MIDP|iPAQ|^LENOVO|^pt-|^zte-|^mot|^sanyo|nokia|\bsharp|kddi|blackberry|^audio|^CDM-|^(siemens|sie-)|(samsung|sec\b|sgh|sch\b|spv)|sonyericsson|palm|wap|^lg|^VX|UC(browser|web)|^Haier|^Gradiente|^PANTECH|^PG-|^sagem|brew|\bnec\b|hiptop|Hutc3G|^amoi|alcatel|Mitsu|BenQ|SoftBank|o2|vodafone|^fly|^inno|^EZZE|foma\b|^sendo|Panasonic|phone|mobile|sms|mms|qtek|^tsm|^ASUS|^IXI|^TIM|^Psion|\d\d\dx\d\d\d|armv|^TCL|^Telit|^UTSTAR|PSP|htc|symbian|windows[ ]?ce|Motorola|PHILIPS|Huawei|^bird|^KWC|^toshiba|^compal|^dbtel|^sam|^grundig|^dallab|^newgen|nintendo|opera[ \d\_\.\-])/i)) {
			return "mobile_light";
		}
		else if(ua.match(/(link|bot|python|curl|php|wget|feed|google|yahoo|bing|yandex|word|http|ruby|crawl|spider|check|jakarta|apache|webdav|nutch|link[ ]?check|acoon|domain|fetch|site|parse|@|url|search|proxy|nikto|follow|stat|scan|client|TYPO3|node|web|bookmark|agent|textmode|api|uri|net|baidu|digg|Catalog|perl|server|PubSub|tweet|news|Mechanize|www|hatena|seo|scour|index|source|scrape|rss|optimi|analy|panscient|looksystems|Shopify-Charlotte|MailChimp|Dispatch|^ning|Blackboard Safeassign|^NG|^scrutiny|^Integrity|^SalesIntelligent|^WhatsApp|Cache|Archive|Mozilla\/[0-9.]+$|^Mozilla\/0.[69]|^Mozilla\/[\d]\.[\d]+ \(compatible[;]?[ a-zA-Z0-9\-_\/\.]*[;]?\)$|Office|^AHC\/2.0|request|^saleslift|^Anemone|^Traackr\.com|^[ a-zA-Z0-9\/\.\-_]+$|^Dart|^HubSpot)/i)) {
			return "seo";
		}
	}
